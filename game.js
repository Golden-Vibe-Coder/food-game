...
function renderStars(){const rating=gameState.rating;let html='';for(let i=0;i<5;i++){const starFill=Math.max(0,Math.min(1,rating-i));if(starFill>=0.99){html+='<span class="star" style="color:#ffc107;">★</span>';}else if(starFill>0.01){const fillPercent=(starFill*100).toFixed(0);html+=`<span class="star" style="position:relative;">★<span style="position:absolute;left:0;top:0;overflow:hidden;width:${fillPercent}%;color:#ffc107;">★</span></span>`;}else{html+='<span class="star">★</span>';}}return html;}
